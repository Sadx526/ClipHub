...
// Lista de anúncios de sites populares
const ads = [
    {
        title: "GitHub",
    ...
    {
        title: "Twitch",
        description: "Streame seus jogos ao vivo",
        url: "https://twitch.tv",
        emoji: ""
    },
    {
        title: "YouTube",
        description: "Videos de jogos e mais",
    ...
    {
        title: "Reddit",
        description: "Comunidades de discussão",
    ...
    },
    {
        ...
        description: "Dúvidas de programação",
        url: "https://stackoverflow.com",
        emoji: ""
    }
];

// Função para gerar anúncios aleatórios
function loadRandomAds() {
    const adsContainer = document.getElementById('ads-container');
    adsContainer.innerHTML = '';
    
    // Pega 4 anúncios aleatórios
    const selectedAds = [];
    const adsPool = [...ads];
    
    for (let i = 0; i < 4; i++) {
        if (adsPool.length === 0) break;
        const randomIndex = Math.floor(Math.random() * adsPool.length);
        const randomAd = adsPool[randomIndex];
        selectedAds.push(randomAd);
        adsPool.splice(randomIndex, 1);
    }
    ...
    selectedAds.forEach(ad => {
        ...
        adElement.innerHTML = `
            <div style="font-size: 1.8rem; margin-bottom: 0.5rem;">${ad.emoji}</div>
...
// Carrega anúncios ao iniciar
document.addEventListener('DOMContentLoaded', () => {
    loadRandomAds();
    
    // Recarrega anúncios a cada 10 segundos
    setInterval(loadRandomAds, 10000);
